...
#define Q 9 // Mô hình D2Q9 (2 chiều, 9 hướng vận tốc)

// Trọng số cho 9 hướng vận tốc trong mô hình D2Q9
...
// Các thành phần vector vận tốc theo trục x và y cho 9 hướng
static const int cx[Q] = {0, 1, 0, -1,  0, 1, -1, -1,  1};
static const int cy[Q] = {0, 0, 1,  0, -1, 1,  1, -1, -1};

// Hướng ngược lại cho mỗi hướng (dùng cho điều kiện biên phản xạ - bounce-back)
static const int opposite[Q] = {0, 3, 4, 1, 2, 7, 8, 5, 6};

/**
 * Đọc cấu hình từ file text.
 * @param filename: Tên file cấu hình.
 * @param nx, ny: Kích thước lưới (ngõ ra).
 * @param nsteps: Số bước thời gian (ngõ ra).
 * @param omega: Tham số thư giãn (ngõ ra).
 * @param u0: Vận tốc đầu vào (ngõ ra).
...
int DocConfig(const char *filename, int *nx, int *ny, int *nsteps, double *omega, double *u0) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    // Đọc định dạng key=value
    if (sscanf(line, "%63[^=]=%63s", key, value) == 2) {
      char *k = key;
      while (*k == ' ' || *k == '\t') k++; // Xóa khoảng trắng đầu
...
/**
 * Khởi tạo trạng thái ban đầu của lưới.
 * Thiết lập hàm phân phối f về trạng thái cân bằng với vận tốc bằng 0.
 * @param f: Mảng hàm phân phối.
 * @param nx, ny: Kích thước lưới.
 */
  ...
  for (x = 0; x < nx; x++) {
    for (y = 0; y < ny; y++) {
      for (i = 0; i < Q; i++) {
        ...
        // Tính hàm phân phối cân bằng feq
...
/**
 * Tính toán các biến vĩ mô (density và velocity) từ hàm phân phối f.
 * @param f: Mảng hàm phân phối đầu vào.
 * @param rho: Mảng mật độ (ngõ ra).
 * @param ux, uy: Mảng vận tốc (ngõ ra).
 */
  ...
  for (x = 0; x < nx; x++) {
    for (y = 0; y < ny; y++) {
      ...
      for (i = 0; i < Q; i++) {
        ...
        r += fi;          // Mật độ rho = tổng của các fi
        vx += fi * cx[i]; // Momentum x = tổng của fi * cxi
        vy += fi * cy[i]; // Momentum y = tổng của fi * cyi
      }
      *(rho + x*ny + y) = r;
      *(ux + x*ny + y) = vx / r;
      *(uy + x*ny + y) = vy / r;
...
/**
 * Thực hiện bước va chạm (Collision) dựa trên mô hình LBGK.
 * @param f: Hàm phân phối hiện tại.
 * @param f_new: Hàm phân phối sau va chạm (ngõ ra).
 * @param rho, ux, uy: Các biến vĩ mô đã tính.
 * @param omega: Tham số thư giãn (liên quan đến độ nhớt).
 */
  ...
  for (x = 0; x < nx; x++) {
    for (y = 0; y < ny; y++) {
      ...
      for (i = 0; i < Q; i++) {
        ...
        double fi = *(f + (x*ny + y)*Q + i);
        // Công thức va chạm: f_new = f - omega * (f - feq)
...
/**
 * Thực hiện bước lan truyền (Streaming).
 * Di chuyển các hạt từ các nút lưới hiện tại sang các nút lân cận theo hướng vận tốc.
 * Xử lý điều kiện biên tường (bounce-back) và chu kỳ (periodic).
 */
  ...
  for (x = 0; x < nx; x++) {
    for (y = 0; y < ny; y++) {
      for (i = 0; i < Q; i++) {
        ...
        // Biên chu kỳ cho trục x
        if (xs < 0) xs = nx - 1;
        if (xs >= nx) xs = 0;
        
        // Điều kiện biên tường (Top/Bottom) - Bounce-back đơn giản
        if (ys < 0 || ys >= ny) {
          int opp = opposite[i];
...
/**
 * Áp dụng điều kiện biên vận tốc cưỡng bức tại inlet và outlet.
 */
void ApDungBienVaoCung(double *ux, int nx, int ny, double u0) {
  int y;
  for (y = 1; y < ny-1; y++) {
    *(ux + 0*ny + y) = u0;         // Cột bên trái
    *(ux + (nx-1)*ny + y) = u0;    // Cột bên phải
  }
}

/**
 * Ghi kết quả mật độ và vận tốc ra file .dat để hậu xử lý.
 */
...
int main(int argc, char **argv) {
  int nx = DEFAULT_NX, ny = DEFAULT_NY, nsteps = DEFAULT_NSTEPS;
  double omega = DEFAULT_OMEGA, u0 = DEFAULT_U0;
  const char *config_file = "config.txt";

  // Kiểm tra tham số dòng lệnh để dùng file config khác
  ...
  // Cấp phát bộ nhớ cho các mảng trường và hàm phân phối
  ...
  for (step = 0; step < nsteps; step++) {
    // Bước 2: Tính đại lượng vĩ mô
    TinhMacro(f, rho, ux, uy, nx, ny);
    // Bước 3: Áp dụng biên vận tốc
    ApDungBienVaoCung(ux, nx, ny, u0);
    // Bước 4: Va chạm (Collision)
    Collision(f, f_new, rho, ux, uy, nx, ny, omega);
    // Bước 5: Lan truyền (Streaming)
  ...
  // Tính lại các đại lượng vĩ mô lần cuối để in ra checksum
  TinhMacro(f, rho, ux, uy, nx, ny);
  double sum_rho = 0.0, sum_ux = 0.0, sum_uy = 0.0, avg_ux = 0.0;
  int x, y;
  for (x = 0; x < nx; x++) {
    for (y = 0; y < ny; y++) {
      sum_rho += *(rho + x*ny + y);
